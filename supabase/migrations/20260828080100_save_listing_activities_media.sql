-- Nested activities + prices, media order/cover/delete in private.save_listing_draft.
...
declare
  ...
  v_kept_price_ids uuid[] := array[]::uuid[];
  v_sort int;
  v_is_active boolean;
  v_status public.directory_status;
  v_applies public.price_applies_to;
  v_category public.price_category;
  v_deleted uuid;
begin
  ...
  -- Nested activities + prices
  if jsonb_typeof(p_payload -> 'activities') = 'array' then
    v_sort := 0;
    for v_activity in
      select value from jsonb_array_elements(p_payload -> 'activities')
    loop
      v_name := btrim(coalesce(v_activity ->> 'name', ''));
      if v_name = '' then
        continue;
      end if;

      v_is_active := coalesce((v_activity ->> 'is_active')::boolean, true);
      v_status := case when v_is_active then 'approved'::public.directory_status
                       else 'archived'::public.directory_status end;
      v_id := nullif(btrim(coalesce(v_activity ->> 'id', '')), '')::uuid;
      v_activity_id := null;

      if v_id is not null then
        update public.listing_activities
        set
          name = v_name,
          short_description = nullif(btrim(coalesce(v_activity ->> 'short_description', '')), ''),
          description = nullif(btrim(coalesce(v_activity ->> 'description', '')), ''),
          duration_minutes = nullif(btrim(coalesce(v_activity ->> 'duration_minutes', '')), '')::int,
          minimum_age = nullif(btrim(coalesce(v_activity ->> 'minimum_age', '')), '')::int,
          maximum_age = nullif(btrim(coalesce(v_activity ->> 'maximum_age', '')), '')::int,
          booking_required = coalesce((v_activity ->> 'booking_required')::boolean, false),
          sort_order = coalesce((v_activity ->> 'sort_order')::int, v_sort),
          status = v_status,
          updated_at = now()
        where id = v_id
      ...
      if v_activity_id is null then
        ...
        insert into public.listing_activities (
          listing_id, name, slug, short_description, description,
          duration_minutes, minimum_age, maximum_age, booking_required,
          is_featured, is_seasonal, status, sort_order
        ) values (
          p_listing_id,
          v_name,
          v_slug,
          nullif(btrim(coalesce(v_activity ->> 'short_description', '')), ''),
          nullif(btrim(coalesce(v_activity ->> 'description', '')), ''),
          nullif(btrim(coalesce(v_activity ->> 'duration_minutes', '')), '')::int,
          nullif(btrim(coalesce(v_activity ->> 'minimum_age', '')), '')::int,
          nullif(btrim(coalesce(v_activity ->> 'maximum_age', '')), '')::int,
          coalesce((v_activity ->> 'booking_required')::boolean, false),
          false,
          false,
          v_status,
          coalesce((v_activity ->> 'sort_order')::int, v_sort)
        )
        returning id into v_activity_id;
      end if;
      ...
      if jsonb_typeof(v_activity -> 'prices') = 'array' then
        for v_price in
          select value from jsonb_array_elements(v_activity -> 'prices')
        loop
          ...
          begin
            v_applies := coalesce(
              nullif(btrim(coalesce(v_price ->> 'applies_to', '')), ''),
              'person'
            )::public.price_applies_to;
          exception when others then
            v_applies := 'person'::public.price_applies_to;
          end;

          begin
            v_category := coalesce(
              nullif(btrim(coalesce(v_price ->> 'price_category', '')), ''),
              'admission'
            )::public.price_category;
          exception when others then
            v_category := 'admission'::public.price_category;
          end;
          ...
          if v_id is not null then
            ...
            set
              listing_activity_id = v_activity_id,
              name = btrim(v_price ->> 'name'),
              standard_price = coalesce(
                nullif(btrim(coalesce(v_price ->> 'standard_price', '')), '')::numeric,
                standard_price
              ),
              ...
              price_category = v_category,
              valid_from = nullif(btrim(coalesce(v_price ->> 'valid_from', '')), '')::date,
              valid_until = nullif(btrim(coalesce(v_price ->> 'valid_until', '')), '')::date,
              is_active = coalesce((v_price ->> 'is_active')::boolean, true),
              sort_order = coalesce((v_price ->> 'sort_order')::int, 0),
              updated_at = now()
            where id = v_id
              and listing_id = p_listing_id
            ...
            if v_id is not null then
              v_kept_price_ids := array_append(v_kept_price_ids, v_id);
            end if;
          else
            insert into public.price_options (
              listing_id, listing_activity_id, name, standard_price, member_price,
              inclusions, applies_to, price_category, valid_from, valid_until,
              ...
              v_applies,
              v_category,
              nullif(btrim(coalesce(v_price ->> 'valid_from', '')), '')::date,
              nullif(btrim(coalesce(v_price ->> 'valid_until', '')), '')::date,
              coalesce((v_price ->> 'is_active')::boolean, true),
              coalesce((v_price ->> 'sort_order')::int, 0)
            )
            returning id into v_id;
            v_kept_price_ids := array_append(v_kept_price_ids, v_id);
          end if;
    ...
    update public.listing_activities
    set status = 'archived'::public.directory_status, updated_at = now()
    where listing_id = p_listing_id
      and (
        cardinality(v_kept_activity_ids) = 0
        or id <> all (v_kept_activity_ids)
      );
    ...
    where listing_id = p_listing_id
      and (
        cardinality(v_kept_price_ids) = 0
        or id <> all (v_kept_price_ids)
      );

  elsif jsonb_typeof(p_payload -> 'prices') = 'array' then
    -- Legacy flat prices path
    for v_price in
  ...
  -- Media delete + reorder / cover
  if jsonb_typeof(p_payload -> 'deleted_media_ids') = 'array' then
    for v_deleted in
      select nullif(btrim(elem), '')::uuid
      from jsonb_array_elements_text(p_payload -> 'deleted_media_ids') as elem
    loop
  ...
  if jsonb_typeof(p_payload -> 'media') = 'array' then
    for v_media in
      select value from jsonb_array_elements(p_payload -> 'media')
    loop
      v_id := nullif(btrim(coalesce(v_media ->> 'id', '')), '')::uuid;
      if v_id is null then
        continue;
      end if;
      update public.listing_media
      set
        sort_order = coalesce((v_media ->> 'sort_order')::int, sort_order),
        is_cover = coalesce((v_media ->> 'is_cover')::boolean, is_cover),
        alt_text = coalesce(nullif(btrim(coalesce(v_media ->> 'alt_text', '')), ''), alt_text)
      where id = v_id
        and listing_id = p_listing_id;
    end loop;
  end if;
